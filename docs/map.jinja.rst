...
We will see a quick setup to configure the ``vault`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``vault`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``vault`` formulas under ``/srv/salt/vault/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/vault/parameters/dns:domain/
    ln -s dns:domain /srv/salt/vault/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/vault/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/vault/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``vault`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``vault`` formulas under ``/srv/salt/vault/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/vault/parameters/roles
...
We create a configuration for the role ``vault/server`` in ``/srv/salt/vault/parameters/roles/vault/server.yaml``:
...
We create another configuration for the role ``vault/client`` in ``/srv/salt/vault/parameters/roles/vault/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles vault/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles vault/client
...
.. code-block:: console
    ...
    mv /srv/salt/vault/parameters/dns:domain/  '/srv/salt/vault/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-vault-as-json:
      test.nop:
        - name: {{ vault | json }}
...
Here is an example based on `template-formula/vault/config/file.sls`_:
    ...
    vault-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='vault-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ vault.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``vault`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ vault['winner'] }}
...
.. _template-formula/vault/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/vault/config/file.sls
