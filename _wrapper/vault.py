...
def delete_secret(path, *args):
    ...
    if args:
        log.debug(f"Affected versions: {' '.join(args)}")
    try:
...
def snapshot(path, backup=False):
    ...
    try:
        ...
        try:
            _check_ret(__salt__["file.mkdir"](str(path.parent)))
            _check_ret(__salt__["file.touch"](str(path)))
            _check_ret(__salt__["file.set_mode"](str(path), "0600"))
            temp_file = _check_ret(__salt__["temp.file"]())
            snapshot_encoded = base64.b64encode(res.content)
            _check_ret(__salt__["hashutil.base64_decodefile"](snapshot_encoded, temp_file))
            _check_ret(__salt__["file.move"](temp_file, str(path)))
        finally:
            try:
