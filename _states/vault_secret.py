"""
Manage Vault secrets statefully.
...
from salt.exceptions import CommandExecutionError, SaltInvocationError, SaltException
    ...
    try:
        ...
        if not __salt__[f"vault.{verb}_secret"](name, **values):
            ...
            raise CommandExecutionError(
                f"Failed to {verb} secret, see logs for details"
            )
...
def absent(name, operation="delete"):
    ...
    for delete/destroy. Will also currently not destroy/wipe a secret that has
    ...
    operation
        ...
        KV v1 secrets will always be wiped since the backend does not support versioning.
    """
    valid_ops = ("delete", "destroy", "wipe")
    if operation not in valid_ops:
        raise SaltInvocationError(
            f"Invalid operation '{operation}'. Valid: {', '.join(valid_ops)}"
        )
    ret = {
        "name": name,
        "result": True,
    ...
    try:
        ...
        if not __salt__[f"vault.{operation}_secret"](name):
            # Only read_secret raises exceptions sadly FIXME?
            raise CommandExecutionError(
                f"Failed to {operation} secret, see logs for details"
            )
